...
void virtual_signal_connect(const char *signal, signal_callback_t callback, void *data)
{
	signal_handler_t *handler = obs_output_get_signal_handler(virtual_out);
	signal_handler_connect(handler, signal, callback, data);
}

void virtual_signal_disconnect(const char *signal, signal_callback_t callback, void *data)
{
	signal_handler_t *handler = obs_output_get_signal_handler(virtual_out);
...
{
	virtual_out_data *data = (virtual_out_data *)bzalloc(sizeof(struct virtual_out_data));
...
static bool virtual_output_start(void *data)
{
	...
	AVPixelFormat fmt = obs_to_ffmpeg_video_format(video_output_get_format(video));
	double fps = video_output_get_frame_rate(video);
	uint64_t interval = static_cast<int64_t>(1000000000 / fps);

	start = shared_queue_create(&out_data->video_queue, out_data->video_mode, fmt,
				    out_data->width, out_data->height, interval,
				    out_data->delay + 10);

	audio_running = shared_queue_create(
		&out_data->audio_queue, ModeAudio, fmt, AUDIO_SIZE, 1, interval,
...
static void virtual_video(void *param, struct video_data *frame)
{
	...
	if (out_data->hori_flip && out_data->flip_ctx.init) {
		...
		shared_queue_push_video(&out_data->video_queue,
					(uint32_t *)out_data->flip_ctx.frame_out->linesize,
					out_data->width, out_data->height,
					out_data->flip_ctx.frame_out->data, frame->timestamp);
		unref_flip_frame(&out_data->flip_ctx);
	} else {
		shared_queue_push_video(&out_data->video_queue, frame->linesize, out_data->width,
					out_data->height, frame->data, frame->timestamp);
	}
	pthread_mutex_unlock(&out_data->mutex);
}
...
static void virtual_audio(void *param, struct audio_data *frame)
{
	...
	shared_queue_push_audio(&out_data->audio_queue, frame->frames * 4, frame->data[0], ts,
				out_data->last_video_ts);
}
...
static void virtual_output_update(void *data, obs_data_t *settings)
{
	...
	if (hori_flip != out_data->hori_flip || keep_ratio != out_data->keep_ratio) {
		pthread_mutex_lock(&out_data->mutex);
...
obs_properties_t *virtual_getproperties(void *data)
{
	...
	obs_properties_add_text(props, "virtual_name", obs_module_text("Virtual.Name"),
				OBS_TEXT_DEFAULT);
...
void virtual_output_init()
{
	...
	virtual_out = obs_output_create("virtual_output", "VirtualOutput", settings, NULL);
...
int video_frame_to_audio_frame(double video_fps, int video_frame, int audio_sample_rate,
