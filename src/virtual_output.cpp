...
static bool virtual_output_start(void *data)
{
	...
	start = shared_queue_create(&out_data->video_queue,
				    out_data->video_mode, fmt, out_data->width,
				    out_data->height, interval,
				    out_data->delay + 10);

	audio_running = shared_queue_create(
		&out_data->audio_queue, ModeAudio, fmt, AUDIO_SIZE, 1, interval,
		video_frame_to_audio_frame(fps, out_data->delay + 10, 44100 * 4,
					   AUDIO_SIZE));
	...
	if (start) {
		...
		if (audio_running) {
			...
			obs_output_set_audio_conversion(out_data->output,
							&conv);
		}
